...
user_t *initUser(string_t id, string_t screenName, string_t token, string_t secretToken, string_t consumerKey, string_t consumerSecretKey)
{
...
void writeUserFile(const user_t *user, const string_t fileName)
{
...
user_t *readUserFile(const string_t fileName)
{
				...
				if (cur)
				{
					...
					if ((!xmlStrcmp(cur->name, (const xmlChar *) "USER")))
					{
						...
						while(cur)
						{
							...
							if ((!xmlStrcmp(cur->name, (const xmlChar *)"screen_name")))
								screenName = xmlNodeListGetString(doc, cur->xmlChildrenNode, 1);

							else if ((!xmlStrcmp(cur->name, (const xmlChar *)"id")))
								id = xmlNodeListGetString(doc, cur->xmlChildrenNode, 1);


							else if ((!xmlStrcmp(cur->name, (const xmlChar *)"consumerKey")))
								consumerKey = xmlNodeListGetString(doc, cur->xmlChildrenNode, 1);


							else if ((!xmlStrcmp(cur->name, (const xmlChar *)"consumerSecretKey")))
								consumerSecretKey = xmlNodeListGetString(doc, cur->xmlChildrenNode, 1);


							else if ((!xmlStrcmp(cur->name, (const xmlChar *)"token")))
								token = xmlNodeListGetString(doc, cur->xmlChildrenNode, 1);


							else if ((!xmlStrcmp(cur->name, (const xmlChar *)"secretToken")))
								secretToken = xmlNodeListGetString(doc, cur->xmlChildrenNode, 1);
